package com.fivecardstud.client;
...
import com.fivecardstud.client.Card.Suit;
import com.fivecardstud.client.GameApi.Delete;
import com.fivecardstud.client.GameApi.EndGame;
import com.fivecardstud.client.GameApi.Operation;
import com.fivecardstud.client.GameApi.Set;
import com.fivecardstud.client.GameApi.SetTurn;
import com.fivecardstud.client.GameApi.SetVisibility;
import com.fivecardstud.client.GameApi.Shuffle;
import com.fivecardstud.client.GameApi.VerifyMove;
import com.fivecardstud.client.GameApi.VerifyMoveDone;
...
public class FiveCardStudLogic {
  ...
  List<Operation> betMove(FiveCardStudState state,int betAmt, List<Integer> playerIds, int lastMovePlayerId)
  {
	  ...
	  int thisTurnId = 0;
	  if (state.stage == BET){
		  nextTurnId = (state.nextTurn - 42 -1 +5 ) % playerIds.size() + 42;
		  thisTurnId = state.nextTurn;
	  }else if(state.stage == DEAL || state.stage == INITDEAL){
		  nextTurnId = (state.nextTurn - 42 - 1 +5) % playerIds.size() + 42; // left of the big blind player id start from 1
		  thisTurnId = state.nextTurn;
	  }

	  if(thisTurnId == state.big){
		  nextTurnId = DEALERID;
  ...
  {
	  List<Operation> operation = Lists.newArrayList();
	  operation.add(new Set(NEXTTURN,dealerId-2));
...
/*	  System.out.println("after sort");
  ...
  public List<Operation> getInitialMove(List<Integer> playerIds,int dealerId)
  {
